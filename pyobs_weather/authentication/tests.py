...
from pyobs_weather.authentication.keycloak import resolve_user
...
class ResolveUserTests(TestCase):
    ...
    def test_new_user_is_created_active(self):
        # Authorization is the PYOBS_AUTH['REQUIRED_GROUPS'] claims gate, not local activation -
        # see pyobs-core's specs/design/shared-authz-keycloak.md.
        user = resolve_user({"sub": "sub-2", "email": "pending@example.org"})
        self.assertTrue(user.is_active)

    def test_new_user_gets_no_staff_or_superuser_flags(self):
    ...
    def test_links_an_existing_user_by_email_on_first_keycloak_login(self):
        ...
        self.assertEqual(KeycloakIdentity.objects.get(user=existing).keycloak_sub, "sub-4")
    ...
    def test_links_an_existing_user_by_username_when_email_does_not_match(self):
        ...
        self.assertEqual(user.pk, existing.pk)
        self.assertEqual(KeycloakIdentity.objects.get(user=existing).keycloak_sub, "sub-5")
